    ...
    apply(ctx, config) {
        ...
        function parseSkillFile(raw) {
            ...
            for (const line of m[1].split(/\r?\n/)) {
                ...
                if (kv[1] === 'name') {
                    ...
                    fm.description = val;
                else if (kv[1] === 'whenToUse' || kv[1] === 'when-to-use')
                    fm.whenToUse = val;
                else if (kv[1] === 'user-invocable' || kv[1] === 'userInvocable')
        ...
        async function scanUserSkills() {
            ...
            for (const ent of entries) {
                ...
                let raw = '';
                try {
                    raw = await fs.readText(await fs.resolve(dir + sep + dirName + sep + 'SKILL.md'));
                }
                catch (e) {
                    continue;
                }
